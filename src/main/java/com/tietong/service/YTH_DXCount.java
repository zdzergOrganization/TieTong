...
public class YTH_DXCount {
	...
	public void dxCount(String KPIMonth, EmployeeType employee, KPIMapper kpiMapper, UploadTablesStatusMapper uploadTablesStatusMapper){
			...
			//先算一体化考核得分
			int yth_khdf = 0;
			List<ZW_YTH_DF> zw_yth_dfs = uploadTablesStatusMapper.getZW_YTH_DF(KPIMonth,employeeName);
			for(ZW_YTH_DF zw_yth_df : zw_yth_dfs){
				if(zw_yth_df.getI() != null)
				{yth_khdf = StringNullToInt.trans(zw_yth_df.getI());}
			}
			kpi_dx.setY(yth_khdf+"");//插入集团客户
			
			//一、 第一大类如果入职为4个月及以上 直接算出实发底薪 其他不用算
			if("y".equals(employeeType) && yth_khdf < 1){
				kpi_dx.setAa((1800*yth_khdf + 200)+"");//插入实发底薪 
			}
			else{
				//二、 第二大类如果入职为4个月以下
				/*//标准底薪
				...
				switch(employeeType){
				case "E" : bz_dx = 266;break;
				case "H" : bz_dx = 400;break;
				case "T" : bz_dx = 533;break;
				default  : bz_dx = 1000;break;
				}
				...
				switch(employeeType){
				case "E" : zf_bt = 66;break;
				case "H" : zf_bt = 100;break;
				case "T" : zf_bt = 133;break;
				default  : zf_bt = 200;break;
				}
				
				//底薪固定
				if(StringUtils.isNotBlank(employee.getEmployeeTypeRel())){
					
					kpi_dx.setL(bz_dx * StringNullToInt.transToDouble(employee.getEmployeeTypeRel()) + "");//标准底薪
					kpi_dx.setM(zf_bt * StringNullToInt.transToDouble(employee.getEmployeeTypeRel()) + "");//住房
					...
					kpi_dx.setL(bz_dx+"");//标准底薪
					kpi_dx.setM(zf_bt+"");//住房
